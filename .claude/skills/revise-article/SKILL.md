...
- **画像プロンプトの管理**：
  - 記事修正で内容が変わったら、画像プロンプトも記事内容に合わせて更新する
  - 記事から削除されたセクションに対応する画像プロンプトファイルは削除する
  - 画像自体が不要になった場合はプロンプトファイルを削除し、記事中のマーカーも削除する
  - ファイル名は挿入箇所がわかる命名にする（`01-header.txt`, `02-body1.txt` 等）
